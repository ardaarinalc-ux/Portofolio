/* ============================================================
   ...
   4. SIMULASI PID-> grafik kendali interaktif (Kp, Ki, Kd)
   ============================================================ */
...
const DATA_TUGAS = [
  { kunci: "COMMISSIONING", isi: "Pengujian operasional instalasi panel (cold/hot test) sebelum serah terima unit ke klien." },
  { kunci: "CONTINUITY TEST", isi: "Verifikasi keterhubungan jalur pengkabelan tanpa short circuit antar fasa dan netral." },
  { kunci: "QC INSPECTION", isi: "Verifikasi kesesuaian material yang terpasang sesuai dengan perancangan awal." },
  { kunci: "QC REPORTING", isi: "Pencatatan deviasi toleransi pada Daily Inspection Checklist berbasis Excel." },
];
...
const DATA_PROYEK = [
  ...
  },
  {
    ...
    gambar: "https://images.unsplash.com/photo-1580983230786-ce385a434707?crop=entropy&cs=srgb&fm=jpg&ixid=M3w4NjA2MDV8MHwxfHNlYXJjaHwyfHxlbGVjdHJvbmljcyUyMGVuZ2luZWVyJTIwbGFib3JhdG9yeSUyMG11bHRpbWV0ZXJ8ZW58MHx8fHwxNzg5MjcwMTI0fDA&ixlib=rb-4.1.0&q=85",
    ...
    specs: [
      ...
      ["Sistem", "1-Fasa 220V, MCB utama 25A"],
      ["Pembagian Beban", "4 grup cabang seimbang (lampu & stop kontak)"],
      ["Proteksi", "MCB + grounding electrode < 5 Ω"],
      ["Output", "Denah instalasi"],
    ],
    galeri: [
      { src: "https://images.unsplash.com/photo-1580983230786-ce385a434707?crop=entropy&cs=srgb&fm=jpg&ixid=M3w4NjA2MDV8MHwxfHNlYXJjaHwyfHxlbGVjdHJvbmljcyUyMGVuZ2luZWVyJTIwbGFib3JhdG9yeSUyMG11bHRpbWV0ZXJ8ZW58MHx8fHwxNzg5MjcwMTI0fDA&ixlib=rb-4.1.0&q=85", label: "Verifikasi catu daya & kalibrasi tegangan saat validasi desain" },
    ],
  },
  {
    ...
    specs: [
      ...
      ["Kalibrasi", "Dashboard threshold ADC real-time"],
    ],
    galeri: [
...
const DATA_SERTIFIKAT = [
  ...
  { id: "<img src='foto/lkmm-pra-td.jpg' alt='LKMM Pra-Dasar' />", judul: "LKTI-Nasional", lengkap: "Lomba karya Tulis Ilmiah Nasional", penerbit: "UIN SATU TULUNGAGUNG",ket:"Juara 3 Lomba Karya Tulis Ilmiah Nasional" },
];
...
function renderStack() {
  document.getElementById("stack-grid").innerHTML = DATA_STACK.map((g) => `
    <div class="kartu-stack reveal">
      <span class="kategori ${g.warna}" style="color:var(--${g.warna === 'kuning' ? 'kuning' : g.warna})">${g.ikon} ${g.kategori}</span>
      <ul>
        ${g.alat.map((a) => `
          <li>
...
function renderRiset() {
  document.getElementById("grid-riset").innerHTML = DATA_RISET.map((r) => `
    <article class="kartu-riset reveal" data-testid="research-card-${r.id}">
...
function renderSertifikat() {
  document.getElementById("grid-sertifikat").innerHTML = DATA_SERTIFIKAT.map((c, i) => `
    <div class="reveal" data-testid="cert-card-${c.id}" onclick="bukaSertifikat(${i})" style="cursor:zoom-in">
...
function renderKanal() {
  // Tampilkan gambar logo jika ada (logoImg), kalau tidak pakai huruf/simbol
  const logoHtml = (k) => k.logoImg ? `<img src="${k.logoImg}" alt="logo ${k.nama}" />` : k.logo;
  ...
  const ikonDock = { wa: "✆", email: "✉", linkedin: "in", github: "⌥" };
  document.getElementById("dock").innerHTML =
    DATA_KANAL.map((k) => `
      <a class="d-${k.kelas}" data-testid="floating-btn-${k.kelas}" href="${k.url}" target="_blank" rel="noopener noreferrer" aria-label="${k.nama}">
        ${k.logoImg ? `<img src="${k.logoImg}" alt="" />` : ikonDock[k.kelas]}<span class="tip">${k.nama}</span>
      </a>`).join("") +
...
function pasangReveal() {
  ...
  // Progress bar riset terisi saat terlihat
  const pengamatBar = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      if (e.isIntersecting) { e.target.style.width = e.target.dataset.progres + "%"; pengamatBar.unobserve(e.target); }
    });
  });
  document.querySelectorAll("[data-progres]").forEach((el) => pengamatBar.observe(el));
}
...
function bukaProyek(i) {
  ...
  // Susun tab: Overview, Visual, (+PID jika ada)
...
function isiTab(j) {
  ...
  if (j === 0) {
    ...
    // Tab 2: denah/SLD (untuk proyek CAD) + galeri foto
    let html = "";
    html += `<div class="galeri">${p.galeri.map((g, k) => `
